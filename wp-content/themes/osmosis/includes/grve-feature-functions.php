 ...
 */
function grve_print_bg_video_container( $bg_video_webm, $bg_video_mp4, $bg_video_ogv, $image_url, $loop, $muted ) {

	$out_video_bg  = $video_attr = '';
	...
	if ( !empty ( $bg_video_webm ) || !empty ( $bg_video_mp4 ) || !empty ( $bg_video_ogv ) ) {
		...
		$out_video_bg .= '  <video preload="auto" autoplay="autoplay"' . $video_attr . '>';
	...
	<div id="grve-feature-section">
		<div class="grve-feature-section-inner" data-item="revslider">
			<div class="grve-feature-element grve-revslider">
				...
				<?php grve_print_feature_go_to_section( $feature_go_to_section, $feature_header_style, 'revslider' ); ?>
			</div>
		</div>
	</div>
<?php
	...
	<div id="grve-feature-section" class="<?php echo esc_attr( $el_class ); ?>" data-effect="<?php echo esc_attr( $feature_effect ); ?>" <?php echo $feature_position['style_height']; ?>>
		...
		<div class="grve-feature-section-inner" data-item="video" <?php echo $feature_position['data_style']; ?>>
			...
			<div id="grve-feature-title" class="grve-feature-content grve-align-<?php echo esc_attr( $text_align ); ?> grve-style-<?php echo esc_attr( $style ); ?> grve-<?php echo esc_attr( $text_animation ); ?>">
				<div class="grve-container">
					...
					<div class="grve-button-wrapper">
						<?php grve_print_feature_button( $button_text, $button_url, $button_type, $button_size, $button_color, $button_shape, $button_target, $button_class ); ?>
						<?php grve_print_feature_button( $button_text2, $button_url2, $button_type2, $button_size2, $button_color2, $button_shape2, $button_target2, $button_class2 ); ?>
					</div>
					<?php } ?>
				</div>
			</div>
			<?php
				...
				grve_print_bg_video_container( $video_webm, $video_mp4, $video_ogv, $video_bg_image, $video_loop, $video_muted );
...
?>
	<div id="grve-feature-section" class="<?php echo esc_attr( $el_class ); ?>" data-effect="<?php echo esc_attr( $feature_effect ); ?>" <?php echo $feature_position['style_height']; ?>>
		...
		<div class="grve-feature-section-inner" data-item="title" <?php echo $feature_position['data_style']; ?>>
			...
			<!-- End Custom Title -->
			<?php grve_print_feature_go_to_section( $feature_go_to_section, $title_color, 'title' ); ?>
		</div>
...
?>
		...
		<?php grve_print_feature_go_to_section( $feature_go_to_section, $title_color, 'slider' ); ?>
	</div>
<?php
...
?>
	<div style="display:none" class="grve-map-point" data-point-lat="<?php echo esc_attr( $map_lat ); ?>" data-point-lng="<?php echo esc_attr( $map_lng ); ?>" data-point-marker="<?php echo esc_attr( $map_marker ); ?>" data-point-title="<?php echo esc_attr( $map_title ); ?>">
		...
		<div class="grve-map-infotext">
			...
			<a class="grve-infotext-link <?php echo esc_attr( $button_class ); ?>" href="<?php echo $button_url; ?>" target="<?php echo $button_target; ?>"><?php echo $button_text; ?></a>
			<?php } ?>
		</div>
		<?php } ?>
	</div>
 ...
 */
function grve_print_feature_go_to_section( $feature_go_to_section, $title_color, $item_type = 'default' ) {

	if( !empty( $feature_go_to_section ) ) {
		if ( 'title' == $item_type ) {
?>
		<div id="grve-feature-goto" class="grve-goto-section grve-icon-nav-down grve-custom-color" data-custom-color="<?php echo esc_attr( $title_color ); ?>"></div>

<?php
		} elseif ( 'slider' == $item_type ) {
?>
		<div id="grve-feature-goto" class="grve-goto-section grve-icon-nav-down grve-<?php echo esc_attr( $title_color ); ?>"></div>
<?php
		} else {
?>
		<div id="grve-feature-goto" class="grve-goto-section grve-icon-nav-down grve-<?php echo esc_attr( $title_color ); ?>"></div>
