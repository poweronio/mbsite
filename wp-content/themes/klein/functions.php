<?php
/**
 * Klein WordPress Theme Function File
 *
 * @version  3.7
 * @author  dunhakdis
 * @package klein
...
/**
 * This will add additional fields to WordPress 
 * editor depending on the chosen post format
 */
...
function klein_setup() {
	...
	add_image_size('klein-thumbnail-large', 850, 450, true); 
...
function klein_widgets_init() {
	 ...
	 */
	register_sidebar( array(
		'name'          => __( 'Page Sidebar Left', 'klein' ),
...
function klein_ie8_fix(){
	$google_fonts = "Source+Sans+Pro:400,600,400italic,600italic";
	?>
		<!--[if lte IE 8]>
...
function klein_scripts(){
	...
	global $wp_version;
		...
		// Global stylesheets
		wp_enqueue_style( 'klein-fontawesome', get_template_directory_uri() . '/css/font-awesome.css', array(), KLEIN_VERSION );
		...
		if( defined('WPB_VC_VERSION') )
		{
			...
			wp_enqueue_style( 'klein-visual-composer-style', get_template_directory_uri() . '/css/visual-composer/visual-composer.css', array( 'js_composer_front' ), KLEIN_VERSION  );
		}
		...
		$is_dark_layout_enable = ot_get_option( 'dark_layout_enable', false );
			...
			$smooth_scroll_enable = ot_get_option('smooth_scroll_enable', 'no');

			if (is_array($smooth_scroll_enable)) 
			{
		...
		if (function_exists('is_buddypress')) {
			if (is_buddypress()) {
				wp_enqueue_script('klein-isotopes', get_template_directory_uri() . '/js/isotope.js', $noDependecy = array('jquery'), KLEIN_VERSION, true);
			}
		}
		// Respond JS
		...
		// Sticky jQuery
		wp_enqueue_script('klein-jquery-js', get_template_directory_uri() . '/js/jquery.sticky.js', array( 'jquery' ), KLEIN_VERSION , true );
		// Template JS
				...
				<div class="row">
					<div class="col-sm-push-6 col-sm-6 login-column">
						<div id="klein-modal-body">
							...
							<?php wp_login_form(); ?>
							<strong>
								<a href="<?php echo wp_lostpassword_url(); ?>" title="<?php _e('Lost Password', 'klein'); ?>">
									<?php _e('Lost Password', 'klein'); ?>
								</a>
							</strong>
					...
					<div class="col-sm-pull-6 col-sm-6 social-connect-column">
						<p><em><?php echo ot_get_option('modal_text', __('Sign in to our website with your favourite social media account. It\'s just one click away','klein')); ?></em></p>
 ...
 */
require_once get_template_directory() . '/bp-template-tags.php';