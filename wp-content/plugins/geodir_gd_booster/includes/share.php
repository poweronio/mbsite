...
namespace geodir_gd_booster // Root namespace.
{
	...
	{
		/**
		 * GD Booster (Shared Methods)
		 *
		 * @package geodir_gd_booster\share
		 * 
		 */
		abstract class share // Shared between {@link advanced_cache} and {@link plugin}.
		{
			...
			public function cache_dir($rel_path = '')
			{
				...
				else if(defined('GEODIR_GD_BOOSTER_DIR') && GEODIR_GD_BOOSTER_DIR)
					$cache_dir = GEODIR_GD_BOOSTER_DIR; // Global constant.
			...
			public function host_dir_token($dashify = FALSE)
			{
				...
				{ // Multisite w/ sub-directories; need a valid sub-directory token.
					...
					if($host_dir_token !== '/' // Perhaps NOT the main site?
					   && (!is_file(($cache_dir = $this->cache_dir()).'/gdb-blog-paths') // NOT a read/valid blog path?
					       || !in_array($host_dir_token, unserialize(file_get_contents($cache_dir.'/gdb-blog-paths')), TRUE))
			...
			public function is_get_request_w_query()
			{
				...
				if(!empty($_GET) || isset($_SERVER['QUERY_STRING'][0]))
					if(!(isset($_GET['gdbABC']) && count($_GET) === 1)) // Special case.
			...
			/* --------------------------------------------------------------------------------------
			 * File/directory iteration utilities for GD Booster.
			 -------------------------------------------------------------------------------------- */
			...
			public function delete_files_from_cache_dir($regex, $check_max_age = FALSE)
			{
				...
				# if(WP_DEBUG) file_put_contents(WP_CONTENT_DIR.'/gdb-debug.log', print_r($regex, TRUE)."\n".print_r($cache_dir_tmp_regex, TRUE)."\n\n", FILE_APPEND);
				...
				foreach($this->dir_regex_iteration($cache_dir_tmp, $cache_dir_tmp_regex) as $_file_dir)
				{
					if(($_file_dir->isFile() || $_file_dir->isLink()) // Files and/or symlinks only.
					   ...
					   // Don't delete files in the immediate directory; e.g. `gdb-advanced-cache` or `.htaccess`, etc.
			 ...
			 *    including `gdb-` prefixed files; or anything else for that matter.
			 *
			...
			/* --------------------------------------------------------------------------------------
			 * Translation utilities for GD Booster.
			 -------------------------------------------------------------------------------------- */
			...
			/* --------------------------------------------------------------------------------------
			 * Hook/filter API for GD Booster.
			 -------------------------------------------------------------------------------------- */
