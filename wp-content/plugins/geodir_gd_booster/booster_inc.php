...
if ( !ini_get('zlib.output_compression') && !defined('DOING_AJAX'))
	@ini_set('zlib.output_compression',2048);
if ( !ini_get('zlib.output_compression_level') )
...
// Start of functions ///////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	*/
	protected function getfilescontents($source = '',$type = '',$recursive = FALSE,$filescontent = '')
	{
		...
		if($type == 'css')
		{
			...
			// updated to work for all type imports format like @import '...', @import "...", @import url(...), @import url('...'), @import url("..."), url(...), url('...'), url("...")
			$import_urls = $this->extract_css_urls($currentfilecontent);
			if ( !empty($import_urls) && isset($import_urls['import']) && !empty($import_urls['import']) ) {
				$treffer = $import_urls['import'];
			} else {
				$treffer[0] = array();
				$treffer[1] = array();
			}
	...
	*/
	protected function css_minify($filescontent = '')
	{
		...
		/* remove comments */
    	$filescontent  = preg_replace( '!/\*[^*]*\*+([^/][^*]*\*+)*/!', '', $filescontent  );

        /* remove tabs, spaces, newlines, etc. */
    	$filescontent  = str_replace( array("\r\n", "\r", "\n", "\t", '  ', '    ', '    '), '', $filescontent );
	...
	*/
	public function css_markup()
	{
		...
		for($j=0;$j<intval($this->css_totalparts);$j++)
		{
			...
			($this->js_closure_compiler ? '&amp;js_cc=1' : '').
	...
	*/
	...
	{
		// If somebody wants to use the included minifier
		if(1==2 && $this->js_hosted_minifier && is_readable($this->js_hosted_minifier_path))
		{
			// Implementation by Vincent Voyer (http://twitter.com/vvoyer)
			// must create tmp files because closure compiler can't work with direct input..
			$tmp_file_path = sys_get_temp_dir().'/'.uniqid();
		...
		else
		{
			...
			// Google Closure has a max limit of 200KB POST size, and will break JS with eval-command
			if(1==2 && $this->js_closure_compiler && strlen($filescontent_urlencoded) < 200000 && preg_match('/[^a-z]eval\(/ism',$filescontent) == 0)
			{
				// Working vars
				$js_minified = '';
			...
			// Switching over to Brendan Eich's JSMinPlus
			else
			{		
			
			
			/* remove comments */
       // $js_minified = preg_replace("/((?:\/\*(?:[^*]|(?:\*+[^*\/]))*\*+\/)|(?:\/\/.*))/", "", $filescontent);
        /* remove tabs, spaces, newlines, etc. */
        //$js_minified = str_replace(array("\r\n","\r","\t","\n",'  ','    ','     '), '',$js_minified);
        /* remove other spaces before/after ) */
       // $js_minified = preg_replace(array('(( )+\))','(\)( )+)'), ')', $js_minified);
        
			
				
				/**
				 * Inclusion of JSMinPlus
				 */
				//include_once('jsmin/JSMinPlus.php');
				//$js_minified = "/* Minified by JSMinPlus */\r\ntry{\r\n".JSMinPlus::minify($filescontent)."\r\nvar boostererror = null;\r\n} catch(e) {}";
				
				include('jsmin/jshrink.php');

				// Basic (default) usage.
				$js_minified = "/* Minified by JShrink */\r\ntry{\r\n".\JShrink\Minifier::minify($filescontent)."\r\nvar boostererror = null;\r\n} catch(e) {}";
			}
		}
	...
	*/
	public function js()
	{
		...
		$identifier = md5(
			...
			intval($this->js_closure_compiler).
		...
		else 
		{
			...
			for($i=0;$i<sizeof($sources);$i++)
			{
				...
				if(preg_match('/\)$/',trim($filescontent))){
			...
			//$this->gdb_write_js_file($cachefile,$filescontent,$interim_cachefile);
			//return $filescontent;
			// Minify
	...
	public function gdb_write_js_file($cachefile,$filescontent,$interim_cachefile){
	...
	*/
	public function js_markup()
	{
		...
		($this->librarydebug ? '&amp;librarydebug=1' : '').
		(!$this->js_minify ? '&amp;js_minify=0' : '').
		($this->js_hosted_minifier ? '&amp;js_cc=1' : '').
	...
	function extract_css_urls( $text )
	{
		...
		foreach ( $matches[3] as $match ) {
			if ( !empty($match) && isset($matches[2][$i]) && !empty($matches[2][$i]) ) {
				$urls['import'][0][] = $matches[2][$i];
				$urls['import'][1][] = preg_replace( '/\\\\(.)/u', '\\1', $match );
			}
			$i++;
		}
		...
		foreach ( $matches[7] as $match ) {
			if ( !empty($match) && isset($matches[6][$i]) && !empty($matches[6][$i]) ) {
				$urls['import'][0][] = $matches[6][$i];
				$urls['import'][1][] = preg_replace( '/\\\\(.)/u', '\\1', $match );
			}
			$i++;
		}
		...
		foreach ( $matches[11] as $match ) {
			if ( !empty($match) && isset($matches[10][$i]) && !empty($matches[10][$i]) ) {
				$urls['property'][0][] = $matches[10][$i];
				$urls['property'][1][] = preg_replace( '/\\\\(.)/u', '\\1', $match );
			}
			$i++;
		}
	...
	/**
	 * Exclude JS/CSS file from combine.
	 */
	function geodir_exclude_js_css()
	{
		$excludes = array('js' => array(), 'css' => array());
		if (defined('GEODIR_GD_BOOSTER_EXCLUDE_COMBINES') && GEODIR_GD_BOOSTER_EXCLUDE_COMBINES!='') {
			$excludes_js_css = GEODIR_GD_BOOSTER_EXCLUDE_COMBINES;
			$excludes_js_css = trim($excludes_js_css);
			$excludes_js_css = explode("\n", $excludes_js_css);
			if (!empty($excludes_js_css)) {
				foreach ( $excludes_js_css as $js_css_file ) {
					$js_css_file = trim($js_css_file);
					if ($js_css_file != '' && substr($js_css_file, strlen($js_css_file) - 3,3) == '.js') {
						$excludes['js'][] = $js_css_file;
					} else if ($js_css_file != '' && substr($js_css_file, strlen($js_css_file) - 4,4) == '.css') {
						$excludes['css'][] = $js_css_file;
					}
				}
			}
		}
