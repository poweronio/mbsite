...
class WPFEPP_Field_Renderer
{
	...
	public function render($args){
		...
		?>
			<input type="text" name="<?php echo $field_name; ?>" value="<?php echo stripslashes(esc_attr($current)); ?>" />
		<?php
		endif;
		...
		if($args['type'] == 'textarea'):
			?>
				<textarea name="<?php echo $field_name ?>"><?php echo stripslashes(esc_textarea($current)); ?></textarea>
			<?php
		endif;
		...
		if($args['type'] == 'multicheckbox'):
			foreach ($args['items'] as $key => $checkboxitem ):
				...
				$item_current = $current[$key];
		?>
			<input type="hidden" name="<?php echo $item_field_name ?>" value="0" />
			<input type="checkbox" name="<?php echo $item_field_name ?>" id="<?php echo $item_field_name ?>" value="1" <?php checked( $item_current ); ?> />
