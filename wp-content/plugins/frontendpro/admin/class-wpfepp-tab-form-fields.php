...
class WPFEPP_Tab_Form_Fields extends WPFEPP_Tab
{
	...
	public function display_item($field_key, $field){
				...
				add_settings_field('element', __( 'Form Element', 'wpfepp-plugin' ), $callback, $page, $section, 
					array_merge(
						array(
							'id' 	=> 'element',
							'type' 	=> 'select',
							'items' => array('input' => __('Text Field', 'wpfepp-plugin'), 'textarea' => __('Text Area', 'wpfepp-plugin'), 'checkbox' => __('Checkbox', 'wpfepp-plugin'), 'select' => __('Select', 'wpfepp-plugin'), 'radio' => __('Radio Buttons', 'wpfepp-plugin'), 'email' => __('Email', 'wpfepp-plugin'), 'url' => __('URL', 'wpfepp-plugin'), 'image_url' => __('Image URL', 'wpfepp-plugin'))
						),
						$args
					)
				);
			...
			?>
				<li class="wpfepp-widget-container <?php echo wpfepp_is_field_supported($field['type'], $this->post_type) ? '' : 'hidden'; ?>">
					...
					<div class="wpfepp-widget-body">
						...
						<input type="hidden" name="form_fields[<?php echo $field_key; ?>][widget_label]" value="<?php echo $field['widget_label']; ?>">
					</div>
				</li>
			<?php
	}
	...
	public function display_items(){
		...
		$fields 			= $form['fields'];
		$this->post_type 	= $form['post_type'];

		foreach($fields as $field_key => $field){
			$this->display_item($field_key, $field);
		}
	}
