...
/**
 * This class is responsible for creating the form management page in the backend area. It displays a list table consisting of user-generated forms, allows users to edit existing forms and create new ones.
 *
 * @since 1.0.0
 * @package WPFEPP
 **/
class WPFEPP_Form_Manager
{
	...
	private function set_defaults(){
		...
		$this->defaults['settings'] = array(
			'no_restrictions' 	=> wpfepp_prepare_default_role_settings(),
			'instantly_publish' => wpfepp_prepare_default_role_settings(),
			'width' 			=> '',
			'redirect_url' 		=> '',
			'button_color' 		=> 'blue',
			'enable_drafts' 	=> true,
	...
	public function add_actions(){
		...
		// The add_actions() function runs at wp_loaded at which time all the WordPress functions required by set_defaults are available.
		$this->set_defaults();
	...
	public function add_menu_item(){
		$this->page_hook = add_menu_page(
		    __('Frontend Publishing Pro', 'wpfepp-plugin'),
		    __('Frontend Publishing Pro', 'wpfepp-plugin'),
		    'manage_options',
		    $this->page,
	...
	public function enqueue($hook) {
		if( $this->page_hook != $hook )
			return;
		wp_enqueue_style("wpfepp-form-manager-stylesheet", plugins_url( "static/css/form-manager.css" , dirname(__FILE__) ), false, $this->version, "all");
	...
	private function generate_default_form_fields(){
		...
		foreach ($this->post_types as $post_type) {
			$default_fields = array(
					...
					'excerpt' 		=> array( 'type' => 'excerpt', 'widget_label' => __('Excerpt', 'wpfepp-plugin'), 'label' => __('Excerpt', 'wpfepp-plugin'), 'enabled' => true, 'required'=> false, 'width' => '', "min_words" => 10, "max_words" => 50, 'strip_tags' => 'all', 'fallback_value' => '', 'prefix_text' => '' ),
					'thumbnail' 	=> array( 'type' => 'thumbnail', 'widget_label' => __('Thumbnail', 'wpfepp-plugin'), 'label' => __('Thumbnail', 'wpfepp-plugin'), 'enabled' => true, 'required'=> false, 'width' => '', 'fallback_value' => '', 'prefix_text' => '' ),
					'formatting' 	=> array( 'type' => 'post_format', 'widget_label' => __('Formats', 'wpfepp-plugin'), 'label' => __('Formats', 'wpfepp-plugin'), 'enabled' => true, 'required'=> false, 'width' => '', 'fallback_value' => '', 'prefix_text' => '' )
				);
	...
	public function delete_forms(){
		...
		if( $_GET['action'] == 'delete' && is_array($_GET['form']) && count($_GET['form']) && isset($_GET['_wpnonce']) && wp_verify_nonce($_GET['_wpnonce'], 'bulk-forms') ){
