...
class GEARS_MODULES_FACEBOOK_LOGIN{
	...
	{
		$errMessage = __('Error: Unable to register your profile. Please make sure you have a valid email address in your Facebook account.', 'gears');
	...
	{
		$errMessage = __('Error: Cannot fetch user data from his/her Facebook Profile. Please make sure your FB App is live and has the right set of permissions.', 'gears');
		$this->wp_error_class($errMessage);
		return;
	}

	public function error_email_exists(){
		$errMessage = __('Error: The username or email you are trying to connect with your facebook account is already registered.', 'gears');
		$this->wp_error_class($errMessage);
		return;
	}
	
	public function error_fb_api(){
		$errMessage = __('There was an error trying to communicate with the Facebook API. Please come back again later.', 'gears');
		wp_error_class($errMessage);
		return;
	}
